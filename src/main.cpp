...
#if defined(NDEBUG)
# error "Deluxcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xb6f0e61884e3a769701750adffbc0ae149f2ae0cdaebf2458ef79d72538b2ccc");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Deluxcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Deluxcoin Signed Message:\n";
...
{
    // Deluxcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Deluxcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Deluxcoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Deluxcoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Deluxcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Deluxcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xb6f0e61884e3a769701750adffbc0ae149f2ae0cdaebf2458ef79d72538b2ccc");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "DELUX COIN";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1517083714;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1393827;
        ...
        assert(block.hashMerkleRoot == uint256("0x5a349e7a11d0053d8cef45114845710c6a501ebb0fff2f494b04718106da4e6a"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Deluxcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DeluxcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DeluxcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DeluxcoinMiner(CWallet *pwallet)
{
    printf("DeluxcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("deluxcoin-miner");
    ...
    try { loop {
        ...
        printf("Running DeluxcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DeluxcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DeluxcoinMiner, pwallet));
}
